...
public class ProductDaoJdbcTemplateImpl implements ProductDao {
	...
	public Product addProduct(Product product) {
		jdbcTemplate.update("insert into product_table(id,name, price, isAvailable) values(?,?,?,?)",
				new Object[] { product.getId(),product.getName(), product.getPrice(), product.getIsAvailable() });
		return product;
	}
	...
	public Product updateProduct(Integer id, Product product) {
		jdbcTemplate.update("update product_table set price =? ,isAvailable=? where id=?",
				new Object[] { product.getPrice(), product.getIsAvailable(), id });
		return product;
	}
}
